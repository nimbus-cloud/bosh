  ...
  module DeploymentPlan
    class InstanceGroupSpecParser
      ...
      def parse(instance_group_spec, options = {})
        ...
        ### nimbus2 specific - start ###
        parse_passive
        parse_drbd
        parse_register_dns
        ### nimbus2 specific - end ###
      ...
      ### nimbus2 specific - start ###
      def parse_passive
        if @instance_group_spec.has_key?('passive')
          if @instance_group_spec['passive']
            @instance_group.passive = :enabled
          else
            @instance_group.passive = :disabled
          end
        else
          @instance_group.passive = :undefined
        end
      end

      def parse_drbd
        if @instance_group_spec['drbd']
          @instance_group.drbd_enabled = safe_property(@instance_group_spec['drbd'], 'enabled', :optional => true, :default => false)
          @instance_group.drbd_force_master = safe_property(@instance_group_spec['drbd'], 'force_master', :optional => true, :default => false)
          @instance_group.drbd_replication_node1 = safe_property(@instance_group_spec['drbd'], 'replication_node1')
          @instance_group.drbd_replication_node2 = safe_property(@instance_group_spec['drbd'], 'replication_node2')
          @instance_group.drbd_replication_type = safe_property(@instance_group_spec['drbd'], 'replication_type')
          @instance_group.drbd_secret = safe_property(@instance_group_spec['drbd'], 'secret')
        else
          @instance_group.drbd_enabled = false
          @instance_group.drbd_force_master = false
          @instance_group.drbd_replication_node1 = ''
          @instance_group.drbd_replication_node2 = ''
          @instance_group.drbd_replication_type = ''
          @instance_group.drbd_secret = ''
        end
      end

      def parse_register_dns
        @instance_group.dns_register_on_start = safe_property(@instance_group_spec, 'dns_register_on_start', :optional => true) || ''
      end
      ### nimbus2 specific - end ###
